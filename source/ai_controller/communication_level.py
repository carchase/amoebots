...
def com_level_main(COM_INPUT, MOV_INPUT, MAIN_INPUT):
    ...
    while True:
        try:
                ...
                if isinstance(response, Message):
                    ...
                    if response.category == 'command' and response.data.get('directive') == 'add':
                        ...
                        botProcess = Process(target=bot_process.bot_process_main,
                                             args=(response.origin, COM_INPUT, processQueue))
            ...
            MAIN_INPUT.put(Message('COM_LEVEL', 'MAIN_LOG', 'error', {'message': str(err)}))
